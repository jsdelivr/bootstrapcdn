...
document=window.document,version="2.1.2",// Define a local copy of jQuery
...
var CURRENT_BOOTSTRAP_VERSION="3.3.1";function compareNums(a,b){return a-b}function isDoctype(node){return node.type==="directive"&&node.name==="!doctype"}var tagNameOf=IN_NODE_JS?function(element){return element.name.toUpperCase()}:function(element){/* @covignore */
...
exports.lintCurrentDocument=function(reporter,disabledIds){this._lint($,reporter,disabledIds)};/**
             ...
             * @returns {undefined} Nothing
             */
exports.showLintReportForCurrentDocument=function(disabledIds){var seenLint=false;var errorCount=0;var reporter=function(lint){var background="background: #"+(lint.id[0]==="W"?"f0ad4e":"d9534f")+"; color: #ffffff;";if(!seenLint){/*eslint-disable no-alert, no-undef, block-scoped-var */
...
seenLint=true}if(!lint.elements.length){console.warn("bootlint: %c "+lint.id+" ",background,lint.message)}else{console.warn("bootlint: %c "+lint.id+" ",background,lint.message+"\n",lint.elements)}errorCount++};this.lintCurrentDocument(reporter,disabledIds);if(errorCount>0){console.info("bootlint: For details, look up the lint problem IDs in the Bootlint wiki: https://github.com/twbs/bootlint/wiki")}};/*eslint-disable no-undef, block-scoped-var */
