...
module.exports=global.document?factory(global,true):function(w){if(!w.document){throw new Error("jQuery requires a window with a document")}return factory(w)}}else{factory(global)}})(typeof window!=="undefined"?window:this,function(window,noGlobal){// Support: Firefox 18+
...
return high!==high||escapedWhitespace?escaped:high<0?String.fromCharCode(high+65536):String.fromCharCode(high>>10|55296,high&1023|56320)},// Used for iframes
// See setDocument()
...
Expr.find["TAG"]=support.getElementsByTagName?function(tag,context){if(typeof context.getElementsByTagName!=="undefined"){return context.getElementsByTagName(tag)}else if(support.qsa){return context.querySelectorAll(tag)}}:function(tag,context){var elem,tmp=[],i=0,// By happy coincidence, a (broken) gEBTN appears on DocumentFragment nodes too
...
if(!aup||!bup){return a===doc?-1:b===doc?1:aup?-1:bup?1:sortInput?indexOf(sortInput,a)-indexOf(sortInput,b):0}else if(aup===bup){return siblingCheck(a,b)}// Otherwise we need full lists of their ancestors for comparison
...
if(jQuery.isFunction(this[match])){this[match](context[match])}else{this.attr(match,context[match])}}}return this}else{elem=document.getElementById(match[2]);// Support: Blackberry 4.6
...
if(status===204||s.type==="HEAD"){statusText="nocontent"}else if(status===304){statusText="notmodified"}else{statusText=response.state;success=response.data;error=response.error;isSuccess=!error}}else{// Extract error from statusText and normalize for non-aborts
...
if(typeof noGlobal===strundefined){window.jQuery=window.$=jQuery}return jQuery})},{}],3:[function(require,module,exports){(function(exports){// export the class if we are in a Node-like system.
...
return c.length});if(!this.set.length){throw new TypeError("Invalid SemVer Range: "+range)}this.format()}Range.prototype.inspect=function(){return'<SemVer Range "'+this.range+'">'};Range.prototype.format=function(){this.range=this.set.map(function(comps){return comps.join(" ").trim()}).join("||").trim();return this.range};Range.prototype.toString=function(){return this.range};Range.prototype.parseRange=function(range){var loose=this.loose;range=range.trim();// `1.2.3 - 1.2.4` => `>=1.2.3 <=1.2.4`
...
gtlt="<";if(xm)M=+M+1;else m=+m+1}ret=gtlt+M+"."+m+"."+p}else if(xm){ret=">="+M+".0.0 <"+(+M+1)+".0.0"}else if(xp){ret=">="+M+"."+m+".0 <"+M+"."+(+m+1)+".0"}return ret})}// Because * is AND-ed with everything else in the comparator,
...
if((!low.operator||low.operator===comp)&&ltefn(version,low.semver)){return false}else if(low.operator===ecomp&&ltfn(version,low.semver)){return false}}return true}// Use the define() function if we're in AMD land
 ...
 */
/*eslint-env node */
var cheerio=require("cheerio");var parseUrl=require("url").parse;var semver=require("semver");var _location=require("./location");var LocationIndex=_location.LocationIndex;(function(exports){"use strict";var NUM_COLS=12;var COL_REGEX=/\bcol-(xs|sm|md|lg)-(\d{1,2})\b/;var COL_REGEX_G=/\bcol-(xs|sm|md|lg)-(\d{1,2})\b/g;var COL_CLASSES=[];var SCREENS=["xs","sm","md","lg"];SCREENS.forEach(function(screen){for(var n=1;n<=NUM_COLS;n++){COL_CLASSES.push(".col-"+screen+"-"+n)}});var SCREEN2NUM={xs:0,sm:1,md:2,lg:3};var NUM2SCREEN=["xs","sm","md","lg"];var IN_NODE_JS=!!cheerio.load;var MIN_JQUERY_VERSION="1.9.1";// as of Bootstrap v3.3.0
var CURRENT_BOOTSTRAP_VERSION="3.3.2";function compareNums(a,b){return a-b}function isDoctype(node){return node.type==="directive"&&node.name==="!doctype"}var tagNameOf=IN_NODE_JS?function(element){return element.name.toUpperCase()}:function(element){/* @covignore */
...
/**
         ...
         */
exports.lintHtml=function(html,reporter,disabledIds){var $=cheerio.load(html,{withStartIndices:true});this._lint($,reporter,disabledIds,html)}}else{// jQuery; in-browser
/* @covignore */
...
window.bootlint=exports})()}})(typeof exports==="object"&&exports||this)},{"./location":1,cheerio:2,semver:3,url:5}],5:[function(require,module,exports){/*eslint-env node, browser */
/* jshint browser: true */
/**
 ...
 */
(function(){"use strict";// Only properties common to both browsers and Node.js are supported.
...
var URL_PROPERTIES=["hash","host","hostname","href","pathname","port","protocol","search"];/**
     ...
     */
function parse(urlStr){var anchor=document.createElement("a");anchor.href=urlStr;var urlObj={};URL_PROPERTIES.forEach(function(property){urlObj[property]=anchor[property]});return urlObj}exports.parse=parse})()},{}]},{},[4]);