...
function getTheme(selected) {
    const { themes } = config.files.bootswatch4;

    selected = selectedTheme(selected);

    return {
        uri: config.files.bootswatch4.bootstrap
                .replace('SWATCH_VERSION', config.files.bootswatch4.version)
...
function generateDataJson() {
    const data = {
        ...
        deprecated: `This file is deprecated and will no longer receive any updates. Visit ${config.app.siteurl}/api for documentation.`,
        timestamp: new Date().toISOString(),
        bootstrap: {},
        fontawesome: {}
    };

    config.files.bootstrap.forEach((bootstrap) => {
    ...
    config.files.fontawesome.forEach((fontawesome) => {
...
const api = {
    ...
    data(name, version = null, endpoint = config.app.apiVersion) {
    ...
    latest(name, endpoint = config.app.apiVersion) {
        return this.tag('latest', name, endpoint);
    },
    message(message, suffix = `Visit ${config.app.siteurl}/api for documentation.`, status = 200) {
        if (suffix) {
            message += ` ${suffix}`;
        }
        return {
    ...
    raw(name, endpoint = config.app.apiVersion) {
    ...
    tag(tag, name, endpoint = config.app.apiVersion) {
        const tags = this.tags(name, endpoint);

        return tags[tag] || null;
    },
    tags(name, endpoint = config.app.apiVersion) {
        const tags = {};
    ...
    versions(name, endpoint = config.app.apiVersion) {
