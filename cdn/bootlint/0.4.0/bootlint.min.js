 ...
 */
var cheerio=require("cheerio");!function(exports){"use strict";function compareNums(a,b){return a-b}function isDoctype(node){return"directive"===node.type&&"!doctype"===node.name}function filenameFromUrl(url){var filename=url.replace(/[#?].*$/,""),lastSlash=filename.lastIndexOf("/");return lastSlash!==-1&&(filename=filename.slice(lastSlash+1)),filename}function withoutClass(classes,klass){return classes.replace(new RegExp("\\b"+klass+"\\b","g"),"")}function columnClassKey(colClass){return SCREEN2NUM[COL_REGEX.exec(colClass)[1]]}function compareColumnClasses(a,b){return columnClassKey(a)-columnClassKey(b)}function sortedColumnClasses(classes){for(var colClasses=[];;){var match=COL_REGEX.exec(classes);if(!match)break;var colClass=match[0];colClasses.push(colClass),classes=withoutClass(classes,colClass)}return colClasses.sort(compareColumnClasses),classes+" "+colClasses.join(" ")}function width2screensFor(classes){for(var width=null,width2screens={};;){var match=COL_REGEX_G.exec(classes);if(!match)break;var screen=match[1];width=match[2];var screens=width2screens[width];screens||(screens=width2screens[width]=[]),screens.push(SCREEN2NUM[screen])}for(width in width2screens)width2screens.hasOwnProperty(width)&&width2screens[width].sort(compareNums);return width2screens}function incrementingRunsFrom(list){list=list.concat([1/0]);for(var runs=[],start=null,prev=null,i=0;i<list.length;i++){var current=list[i];null===start?start=current:prev+1!==current&&(start!==prev&&runs.push([start,prev]),start=current),prev=current}return runs}function LintError(id,message){this.id=id,this.message=message}function LintWarning(id,message){this.id=id,this.message=message}function addLinter(id,linter){function linterWrapper($,reporter){function specializedReporter(message){reporter(new Problem(id,message))}linter($,specializedReporter)}if(allLinters[id])throw new Error("Linter already registered with ID: "+id);var Problem=null;if("E"===id[0])Problem=LintError;else{if("W"!==id[0])throw new Error("Invalid linter ID: "+id);Problem=LintWarning}linterWrapper.id=id,allLinters[id]=linterWrapper}var NUM_COLS=12,COL_REGEX=/\bcol-(xs|sm|md|lg)-(\d{0,2})\b/,COL_REGEX_G=/\bcol-(xs|sm|md|lg)-(\d{0,2})\b/g,COL_CLASSES=[],SCREENS=["xs","sm","md","lg"];SCREENS.forEach(function(screen){for(var n=1;n<=NUM_COLS;n++)COL_CLASSES.push(".col-"+screen+"-"+n)});var SCREEN2NUM={xs:0,sm:1,md:2,lg:3},NUM2SCREEN=["xs","sm","md","lg"],IN_NODE_JS=!!cheerio.load;exports.LintError=LintError,exports.LintWarning=LintWarning;var allLinters={};addLinter("E001",function(){var MISSING_DOCTYPE="Document is missing a DOCTYPE declaration",NON_HTML5_DOCTYPE="Document declares a non-HTML5 DOCTYPE";return IN_NODE_JS?function($,reporter){for(var doctype=$(":root")[0];doctype&&!isDoctype(doctype);)doctype=doctype.prev;if(!doctype)return void reporter(MISSING_DOCTYPE);var doctypeId=doctype.data.toLowerCase();"!doctype html"!==doctypeId&&'!doctype html system "about:legacy-compat"'!==doctypeId&&reporter(NON_HTML5_DOCTYPE)}:function($,reporter){var doc=window.document;null===doc.doctype?reporter(MISSING_DOCTYPE):doc.doctype.publicId?reporter(NON_HTML5_DOCTYPE):doc.doctype.systemId&&"about:legacy-compat"!==doc.doctype.systemId&&reporter(NON_HTML5_DOCTYPE)}}()),addLinter("W001",function($,reporter){var meta=$("head>meta[charset]"),charset=meta.attr("charset");charset?"utf-8"!==charset.toLowerCase()&&reporter("charset <meta> tag is specifying a legacy, non-UTF-8 charset"):reporter("<head> is missing UTF-8 charset <meta> tag")}),addLinter("W002",function($,reporter){var meta=$('head>meta[http-equiv="X-UA-Compatible"][content="IE=edge"]');meta.length||reporter("<head> is missing X-UA-Compatible <meta> tag that disables old IE compatibility modes")}),addLinter("W003",function($,reporter){var meta=$('head>meta[name="viewport"][content]');meta.length||reporter("<head> is missing viewport <meta> tag that enables responsiveness")}),addLinter("E002",function($,reporter){for(var columnClasses=[],n=1;n<=12;n++)columnClasses.push(".span"+n);var selector=columnClasses.join(","),spanNs=$(selector);spanNs.length&&reporter("Found one or more uses of outdated Bootstrap v2 `.spanN` grid classes")}),addLinter("E003",function($,reporter){var notAnyColClass=COL_CLASSES.map(function(colClass){return":not("+colClass+")"}).join(""),selector="*"+notAnyColClass+">.row",rowsOutsideColumns=$(selector),rowsOutsideColumnsAndContainers=rowsOutsideColumns.filter(function(){for(var parent=$(this).parent();parent.length;){if(parent.is(".container, .container-fluid"))return!1;parent=$(parent).parent()}return!0});rowsOutsideColumnsAndContainers.length&&reporter("Found one or more `.row`s that were not children of a grid column or descendants of a `.container` or `.container-fluid`")}),addLinter("E004",function($,reporter){var nestedContainers=$(".container, .container-fluid").children(".container, .container-fluid");nestedContainers.length&&reporter("Containers (`.container` and `.container-fluid`) are not nestable")}),addLinter("E005",function($,reporter){var selector=COL_CLASSES.map(function(col){return".row"+col}).join(","),rowCols=$(selector);rowCols.length&&reporter("Found both `.row` and `.col-*-*` used on the same element")}),addLinter("W004",function($,reporter){var remoteModalTriggers=$('[data-toggle="modal"][data-remote]');remoteModalTriggers.length&&reporter("Found one or more modals using the deprecated `remote` option")}),addLinter("W005",function($,reporter){var theWindow=null;try{theWindow=window}catch(e){}if(!theWindow||!theWindow.$&&!theWindow.jQuery){var jqueries=$('script[src*="jquery"],script[src*="jQuery"]');jqueries.length||reporter("Unable to locate jQuery, which is required for Bootstrap's JavaScript plugins to work")}}),addLinter("E006",function($,reporter){var selectInputGroups=$(".input-group select");selectInputGroups.length&&reporter("`.input-group` contains a <select>; this should be avoided as <select>s cannot be fully styled in WebKit browsers");var textareaInputGroups=$(".input-group textarea");textareaInputGroups.length&&reporter("`.input-group` contains a <textarea>; only text-based <input>s are permitted in an `.input-group`")}),addLinter("E007",function($,reporter){var longhands=$('script[src*="bootstrap.js"]').filter(function(i,script){var url=$(script).attr("src"),filename=filenameFromUrl(url);return"bootstrap.js"===filename});if(longhands.length){var minifieds=$('script[src*="bootstrap.min.js"]').filter(function(i,script){var url=$(script).attr("src"),filename=filenameFromUrl(url);return"bootstrap.min.js"===filename});minifieds.length&&reporter("Only one copy of Bootstrap's JS should be included; currently the webpage includes both bootstrap.js and bootstrap.min.js")}}),addLinter("W006",function($,reporter){var selector=['[disabled][data-toggle="tooltip"]','.disabled[data-toggle="tooltip"]','[disabled][data-toggle="popover"]','.disabled[data-toggle="popover"]'].join(","),disabledWithTooltips=$(selector);disabledWithTooltips.length&&reporter("Tooltips and popovers on disabled elements cannot be triggered by user interaction unless the element becomes enabled. To have tooltips and popovers be triggerable by the user even when their associated element is disabled, put the disabled element inside a wrapper <div> and apply the tooltip or popover to the wrapper <div> instead.")}),addLinter("E008",function($,reporter){var nonBodyContainers=$('.btn-group [data-toggle="tooltip"]:not([data-container="body"]), .btn-group [data-toggle="popover"]:not([data-container="body"])');nonBodyContainers.length&&reporter("Tooltips and popovers within button groups should have their `container` set to 'body'. Found tooltips/popovers that might lack this setting.")}),addLinter("E009",function($,reporter){var selector=[".input-group:not(.input-group-lg) .btn-lg",".input-group:not(.input-group-lg) .input-lg",".input-group:not(.input-group-sm) .btn-sm",".input-group:not(.input-group-sm) .input-sm"].join(","),badInputGroupSizing=$(selector);badInputGroupSizing.length&&reporter("Button and input sizing within `.input-group`s can cause issues. Instead, use input group sizing classes `.input-group-lg` or `.input-group-sm`")}),addLinter("E010",function($,reporter){var badInputGroups=$(".input-group").filter(function(i,inputGroup){return $(inputGroup).find(".form-control").length>1});badInputGroups.length&&reporter("Input groups cannot contain multiple `.form-control`s")}),addLinter("E011",function($,reporter){var badMixes=$(".input-group.form-group");badMixes.length&&reporter(".input-group and .form-group cannot be used directly on the same element. Instead, nest the .input-group within the .form-group")}),addLinter("E012",function($,reporter){var selector=COL_CLASSES.map(function(colClass){return".input-group"+colClass}).join(","),badMixes=$(selector);badMixes.length&&reporter(".input-group and .col-*-* cannot be used directly on the same element. Instead, nest the .input-group within the .col-*-*")}),addLinter("E013",function($,reporter){var ALLOWED_CHILD_CLASSES=COL_CLASSES.concat([".clearfix",".bs-customizer-input"]),selector=".row>*"+ALLOWED_CHILD_CLASSES.map(function(colClass){return":not("+colClass+")"}).join(""),nonColRowChildren=$(selector);nonColRowChildren.length&&reporter("Only columns (.col-*-*) may be children of `.row`s")}),addLinter("E014",function($,reporter){var selector=COL_CLASSES.map(function(colClass){return"*:not(.row):not(.form-group)>"+colClass+":not(col):not(th):not(td)"}).join(","),colsOutsideRowsAndFormGroups=$(selector);colsOutsideRowsAndFormGroups.length&&reporter("Columns (.col-*-*) can only be children of `.row`s or `.form-group`s")}),addLinter("E015",function($,reporter){var addOnClasses=[".input-group-addon",".input-group-btn"],combos=[];addOnClasses.forEach(function(first){addOnClasses.forEach(function(second){combos.push(".input-group>"+first+"+"+second)})});var selector=combos.join(","),multipleAddOns=$(selector);multipleAddOns.length&&reporter("Having multiple add-ons on a single side of an input group is not supported")}),addLinter("E016",function($,reporter){var badBtnToggle=$(".btn.dropdown-toggle ~ .btn");badBtnToggle.length&&reporter("`.btn.dropdown-toggle` must be the last button in a button group.")}),addLinter("W007",function($,reporter){var badBtnType=$('button:not([type="submit"], [type="reset"], [type="button"])');badBtnType.length&&reporter("Always set a `type` on `<button>`s.")}),addLinter("E017",function($,reporter){var badCheckboxes=$(".checkbox").filter(function(i,div){return $(div).filter(':has(>label>input[type="checkbox"])').length<=0});badCheckboxes.length&&reporter('Incorrect markup used with the `.checkbox` class. The correct markup structure is .checkbox>label>input[type="checkbox"]')}),addLinter("E018",function($,reporter){var badRadios=$(".radio").filter(function(i,div){return $(div).filter(':has(>label>input[type="radio"])').length<=0});badRadios.length&&reporter('Incorrect markup used with the `.radio` class. The correct markup structure is .radio>label>input[type="radio"]')}),addLinter("E019",function($,reporter){var wrongElems=$(".checkbox-inline:not(label)");wrongElems.length&&reporter(".checkbox-inline should only be used on <label> elements");var badStructures=$(".checkbox-inline").filter(function(i,label){return $(label).children('input[type="checkbox"]').length<=0});badStructures.length&&reporter('Incorrect markup used with the `.checkbox-inline` class. The correct markup structure is label.checkbox-inline>input[type="checkbox"]')}),addLinter("E020",function($,reporter){var wrongElems=$(".radio-inline:not(label)");wrongElems.length&&reporter(".radio-inline should only be used on <label> elements");var badStructures=$(".radio-inline").filter(function(i,label){return $(label).children('input[type="radio"]').length<=0});badStructures.length&&reporter('Incorrect markup used with the `.radio-inline` class. The correct markup structure is label.radio-inline>input[type="radio"]')}),addLinter("E021",function($,reporter){var selector=['[data-toggle="buttons"]>label:not(.active)>input[type="checkbox"][checked]','[data-toggle="buttons"]>label.active>input[type="checkbox"]:not([checked])','[data-toggle="buttons"]>label:not(.active)>input[type="radio"][checked]','[data-toggle="buttons"]>label.active>input[type="radio"]:not([checked])'].join(","),mismatchedButtonInputs=$(selector);mismatchedButtonInputs.length&&reporter(".active class used without the `checked` attribute (or vice-versa) in a button group using the button.js plugin")}),addLinter("E022",function($,reporter){var badNestings=$(".table .modal");badNestings.length&&reporter("Modal markup should not be placed within other components, so as to avoid the component's styles interfering with the modal's appearance or functionality")}),addLinter("E023",function($,reporter){var badPanelBody=$(".panel-body").parent(":not(.panel, .panel-collapse)");badPanelBody.length&&reporter("`.panel-body` must have a `.panel` or `.panel-collapse` parent")}),addLinter("E024",function($,reporter){var badPanelHeading=$(".panel-heading").parent(":not(.panel)");badPanelHeading.length&&reporter("`.panel-heading` must have a `.panel` parent")}),addLinter("E025",function($,reporter){var badPanelFooter=$(".panel-footer").parent(":not(.panel, .panel-collapse)");badPanelFooter.length&&reporter("`.panel-footer` must have a `.panel` or `.panel-collapse` parent")}),addLinter("E026",function($,reporter){var badPanelTitle=$(".panel-title").parent(":not(.panel-heading)");badPanelTitle.length&&reporter("`.panel-title` must have a `.panel-heading` parent")}),addLinter("E027",function($,reporter){var badStructure=$(".table.table-responsive,table.table-responsive");badStructure.length&&reporter("`.table-responsive` is supposed to be used on the table's parent wrapper <div>, not on the table itself")}),addLinter("E028",function($,reporter){var ancestorsMissingClasses=$(".form-control-feedback").filter(function(){return 1!==$(this).closest(".form-group.has-feedback").length});ancestorsMissingClasses.length&&reporter("`.form-control-feedback` must have a `.form-group.has-feedback` ancestor")}),addLinter("E029",function($,reporter){var columns=$(COL_CLASSES.join(","));columns.each(function(_index,column){for(var classes=$(column).attr("class"),simplifiedClasses=classes,width2screens=width2screensFor(classes),isRedundant=!1,width=1;width<=NUM_COLS;width++){var screens=width2screens[width];if(screens){var runs=incrementingRunsFrom(screens);if(runs.length){isRedundant=!0;for(var i=0;i<runs.length;i++)for(var run=runs[i],min=run[0],max=run[1],screenNum=min+1;screenNum<=max;screenNum++){var colClass="col-"+NUM2SCREEN[screenNum]+"-"+width;simplifiedClasses=withoutClass(simplifiedClasses,colClass)}}}}if(isRedundant){simplifiedClasses=sortedColumnClasses(simplifiedClasses),simplifiedClasses=simplifiedClasses.replace(/ {2,}/g," ").trim();var oldClass='class="'+classes+'"',newClass='class="'+simplifiedClasses+'"';reporter("Since grid classes apply to devices with screen widths greater than or equal to the breakpoint sizes (unless overridden by grid classes targeting larger screens), "+oldClass+" is redundant and can be simplified to "+newClass)}})}),addLinter("E030",function($,reporter){var missingGlyphiconClass=$('[class*="glyphicon-"]:not(.glyphicon):not(.glyphicon-class)').filter(function(){return/\bglyphicon-([a-zA-Z]+)\b/.test($(this).attr("class"))});missingGlyphiconClass.length&&reporter("Found elements with a .glyphicon-* class that were missing the additional required .glyphicon class.")}),addLinter("E031",function($,reporter){$(".glyphicon:not(:empty)").length&&reporter("Glyphicon classes must only be used on elements that contain no text content and have no child elements.")}),addLinter("E032",function($,reporter){$(".modal-dialog").parent(":not(.modal)").length&&reporter(".modal-dialog must be a child of .modal"),$(".modal-content").parent(":not(.modal-dialog)").length&&reporter(".modal-content must be a child of .modal-dialog"),$(".modal-header").parent(":not(.modal-content)").length&&reporter(".modal-header must be a child of .modal-content"),$(".modal-body").parent(":not(.modal-content)").length&&reporter(".modal-body must be a child of .modal-content"),$(".modal-footer").parent(":not(.modal-content)").length&&reporter(".modal-footer must be a child of .modal-content"),$(".modal-title").parent(":not(.modal-header)").length&&reporter(".modal-title must be a child of .modal-header")}),exports._lint=function($,reporter,disabledIdList){var disabledIdSet={};disabledIdList.forEach(function(disabledId){disabledIdSet[disabledId]=!0}),Object.keys(allLinters).sort().forEach(function(linterId){disabledIdSet[linterId]||allLinters[linterId]($,reporter)})},IN_NODE_JS?exports.lintHtml=function(html,reporter,disabledIds){var $=cheerio.load(html);this._lint($,reporter,disabledIds)}:!function(){var $=cheerio;exports.lintCurrentDocument=function(reporter,disabledIds){this._lint($,reporter,disabledIds)},exports.showLintReportForCurrentDocument=function(disabledIds){var seenLint=!1,reporter=function(lint){seenLint||(window.alert("bootlint found errors in this document! See the JavaScript console for details."),seenLint=!0),console.warn("bootlint:",lint.id,lint.message)};this.lintCurrentDocument(reporter,disabledIds)},window.bootlint=exports,$(function(){exports.showLintReportForCurrentDocument([])})}()}("object"==typeof exports&&exports||this)},{cheerio:1}]},{},[2]);
